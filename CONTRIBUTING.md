...
The release notes are generated by `git-cliff` from `cliff.toml` and handed to
goreleaser, which publishes them as the GitHub release body. `make
...
Put a `!` after the type *and* a `BREAKING CHANGE:` footer in its own
paragraph, separated by a blank line from any trailers:

```
feat!: drop the v1 service schema

Body explaining the change.

BREAKING CHANGE: `version: "1"` documents no longer load; migrate with
`nix-compose migrate`.

Co-Authored-By: Someone <someone@example.com>
```

Three tools have to agree on that shape, and only this one satisfies all
three:

- **gsemver** picks up `BREAKING CHANGE:` but *not* the hyphenated
  `BREAKING-CHANGE:`, so the hyphenated form silently fails to bump.
- **git trailers** cannot parse a token containing a space, so putting
  `BREAKING CHANGE:` adjacent to `Co-Authored-By:` invalidates the whole
  trailer block. The blank line keeps the trailers parseable.
